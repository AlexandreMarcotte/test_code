...
class MultiChannelsPyQtGraph(object):
    def __init__(self, data_queue, time_queue, N_DATA=500, N_SIGNALS=8):
        """
    ...
    def update_eeg_plotting(self):
        # Using chunk to avoid lagging behind time by having a displaying rate
    ...
    def update_fft_plotting(self):
        ...
        if len(self.last_queue_val) == 100:
            ...
            self.curve_freq.setData(2.0 / len(data_freq) * np.abs(yf[:len(data_freq) // 2]))
...
class CreateFakeData(threading.Thread):
    ...
    def run(self):
        ...
        while 1:
            data_queue.put([random() for _ in range(8)])
            time_queue.put(time.time())
            time.sleep(0.01)


if __name__ == '__main__':
    time_queue = multiprocessing.Queue(maxsize=500)
    data_queue = multiprocessing.Queue(maxsize=500)
    # Create fake data for test case
    create_fake_data = CreateFakeData(data_queue, time_queue)
